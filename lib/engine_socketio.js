...
SocketIoEngine.prototype.step = function (requestSpec, ee) {
  ...
  function preStep(context, callback){
    ...
    let emitSpec = {
      namespace: requestSpec.emit.namespace,
      reinit: requestSpec.emit.reinit,
      headers: { cookie: requestSpec.emit.cookie, headers: requestSpec.emit.headers }
    };

    self.loadContextSocket(emitSpec, self.config, context, function(err, socket){
      if(err) {
        debug(err);
...
SocketIoEngine.prototype.loadContextSocket = function(emitSpec, config, context, cb) {
  context.sockets = context.sockets || {};

  let namespace = emitSpec.namespace;
  let reinit = emitSpec.reinit;
  let emitHeaders = emitSpec.headers;

  if(!context.sockets[namespace] || reinit) {
    ...
    let headers = engineUtil.createHeader(config, emitHeaders || {}, context);
    ...
    if (reinit && context.sockets[namespace]) {
...
SocketIoEngine.prototype.compile = function (tasks, scenarioSpec, ee) {
  ...
  function zero(callback, context) {
    ...
    self.loadContextSocket(emitSpec, config, context, function done(err) {
      if (err) {
        ee.emit('error', err);
        return callback(err, context);
      }
