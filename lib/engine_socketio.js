...
SocketIoEngine.prototype.step = function (requestSpec, socketHeaders, ee) {
  ...
  function preStep(context, callback){
    ...
    let defaultHeaders = engineUtil.lowcaseKeys(
      (self.config.defaults && self.config.defaults.headers) ?
        self.config.defaults.headers : {'user-agent': USER_AGENT});
    ...
    let defaultCookie = self.config.defaults ? self.config.defaults.cookie || {} : {};
    let cookie = _.reduce(
      userCookies,
      function(acc, v, k) {
        acc[k] = v;
...
SocketIoEngine.prototype.compile = function (tasks, scenarioSpec, ee) {
  ...
  function zero(callback, context) {
    ...
    ee.emit('started');
    self.loadContextSocket('/', scenarioSpec.socketHeaders, context, function done(err) {
      if (err) {
        ee.emit('error', err);
        return callback(err, context);
      }
