...
module.exports = {
  ...
  captureOrMatch,
  evil: evil,
  lowcaseKeys: lowcaseKeys,
  _renderVariables: renderVariables
};

function createHeader(config, configHeaders, context) {
  ...
  let userHeaders = configHeaders.headers || {};
  let customHeaders = L.extend(defaultHeaders,
                                   lowcaseKeys(userHeaders));
  let headers = L.reduce(customHeaders,
                        function(acc, v, k) {
                          acc[k] = template(v, context);
                          return acc;
                        }, {});

  let userCookies = configHeaders.cookie || {};
